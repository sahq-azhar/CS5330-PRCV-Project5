...
def train_network(epoch, network, train_loader, optimizer, train_losses, train_counter, log_interval):
  network.train()
...
def test_network(network, test_loader, test_losses):
  ...
  print('\nAverage loss(test set): {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
...
def main(argv):
    ...
    network = Net()
    optimizer = to.SGD(network.parameters(), lr=learning_rate,
                          momentum=momentum)
    ...
    for epoch in range(1, n_epochs + 1):
      train_network(epoch, network, train_loader, optimizer, train_losses, train_counter, log_interval)
      test_network(network, test_loader, test_losses)
